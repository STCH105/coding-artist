class kirby{
  kirby(float a, float b, float c) {x=a; y=b; z=c; vx=random(-5,5); vy=random(-5,5);}
  kirby(float a, float b, float c, int num) 
    {x=a; y=b; z=c; vx=random(-5,5); vy=random(-5,5); watt= new waddle(a+250/2/c,b,c);}
  float x,y,z, vx,vy;
  waddle watt;
